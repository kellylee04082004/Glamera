(() => {
    ...
    function applyCanvasFilter(ctx, filterName) {
      if (filterName === "bw") {
        ctx.filter = "grayscale(1) contrast(1.35) brightness(1.03)";
        return;
      }
      if (filterName === "rio") {
        ctx.filter = "saturate(1.65) contrast(1.12) brightness(1.08) hue-rotate(-6deg)";
        return;
      }
      ctx.filter = "none";
    }
    ...
    async function captureFilteredFramePortrait() {
      ...
      // Crop source to portrait aspect 3:4
    ...
    async function buildPhotoboothStripBlack(photos) {
      ...
      // Strict black frame style, portrait tiles
      const stripW = 780;
      const outer = 18;
      const divider = 26;
      ...
      // Solid black base like reference
      ...
      imgs.forEach((im, i) => {
        ...
        // Subtle bevel edge inside black
        ctx.save();
        ...
        // Strong black line like physical frame
        ctx.strokeStyle = "rgba(0,0,0,0.9)";
        ctx.lineWidth = 10;
        ctx.strokeRect(x + 3, y + 3, tileW - 6, tileH - 6);
  
        // Divider bar between frames
        if (i < 3) {
          ctx.fillStyle = "#0b0b0b";
